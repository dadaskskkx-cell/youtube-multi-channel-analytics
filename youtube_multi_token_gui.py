...
class YouTubeManagerApp:
    ...
    def authorize_channel(self) -> None:
        def job() -> None:
            ...
            row = save_authorized_channel(
                client_secrets=defaults["client_secrets"],
                token_dir=defaults["token_dir"],
                registry_path=defaults["registry"],
                alias=alias,
                force_reauth=True,
                port=port,
            )
    ...
    def refresh_registry(self) -> None:
        try:
            defaults = get_default_paths()
            df = list_channels(defaults["registry"])
        except Exception as exc:
    ...
    def disable_selected(self) -> None:
        ...
        def job() -> None:
            ...
            result = disable_channel(
                ...
                move_token=False,
                inactive_dir=get_default_paths()["inactive_dir"],
            )
            self.root.after(0, self.refresh_registry)
